...
# .ExternalHelp PSADSyncTool-Help.xml
...
function New-CompanyAdUser {
	...
	(
		...
		[Parameter()]
		[ValidateNotNullOrEmpty()]
		[string]$Path = (GetPSADSyncToolConfiguration).NewUserCreation.Path,
		...
		[Parameter()]
		[ValidateNotNullOrEmpty()]
		[string]$UsernamePattern = (GetPSADSyncToolConfiguration).NewUserCreation.AccountNamePattern
	)
...
function InvokeUserTermination {
	...
	switch ((GetPSADSyncToolConfiguration).UserTermination.Action) {
		'Disable' {
...
function TestUserTerminated {
	...
	$csvField = (GetPSADSyncToolConfiguration).UserTermination.FieldValueSettings.CsvField
	$csvValue = (GetPSADSyncToolConfiguration).UserTermination.FieldValueSettings.CsvValue
...
function TestIsUserTerminationEnabled {
	...
	if ((GetPSADSyncToolConfiguration).UserTermination.Enabled) {
		$true
	} else {
		$false
	}
...
function TestIsUserCreationEnabled {
	...
	(GetPSADSyncToolConfiguration).UserCreation.Enabled
}
...
function WriteLog {
	...
	(
		[Parameter()]
		[ValidateNotNullOrEmpty()]
		[string]$FilePath = "$PSScriptRoot\PSADSyncTool.csv",
...
function SendStaleAccountEmail {
	...
	(
		...
		[Parameter()]
		[ValidateNotNullOrEmpty()]
		[string]$Subject = (GetPSADSyncToolConfiguration).Email.Templates.UnusedAccount.Subject,

		[Parameter()]
		[ValidateNotNullOrEmpty()]
		[string]$FromEmailAddress = (GetPSADSyncToolConfiguration).Email.Templates.UnusedAccount.FromEmailAddress,

		[Parameter()]
		[ValidateNotNullOrEmpty()]
		[string]$FromEmailName = (GetPSADSyncToolConfiguration).Email.Templates.UnusedAccount.FromEmailName,

		[Parameter()]
		[ValidateNotNullOrEmpty()]
		[string]$SmtpServer = (GetPSADSyncToolConfiguration).Email.SmtpServer
	...
	process {
		try {
			...
			$emailBody = $emailBody -f $managerEmail, $AdUser.Name, (GetPSADSyncToolConfiguration).CompanyName
...
function TestShouldCreateNewUser {
	...
	if ((TestIsUserTerminationEnabled) -and (TestUserTerminated -CsvUser $CsvUser)) {
		...
		if ($csvfield = (GetPSADSyncToolConfiguration).NewUserCreation.Exclude.FieldValueSettings.CsvField) {
			$csvValue = (GetPSADSyncToolConfiguration).NewUserCreation.Exclude.FieldValueSettings.CsvValue
			if ($CsvUser.$csvField -in $csvValue) {
				$false
			} else {
				$true
